...
export const SMTPConfigModal: React.FC<Props> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/75 backdrop-blur-sm p-3 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-3xl max-w-xl w-full p-5 sm:p-7 shadow-2xl border border-slate-200 relative my-6 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center space-x-3 mb-5 pr-8">
          <div className="p-3 bg-gradient-to-br from-amber-500 to-amber-700 text-white rounded-2xl shadow-md">
        ...
        {!adminEmail && (
          <div className="mb-5 bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center justify-between">
            ...
            <button
              ...
              className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold rounded-xl shadow transition-colors flex-shrink-0 ml-2"
            >
        ...
        {/* Status Indicator */}
        <div className="mb-4 flex flex-wrap items-center justify-between bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs gap-2">
          <div className="flex items-center space-x-2">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
            <span className="text-slate-600 font-medium">
              {lang === 'zh' ? '當前發信模式：' : 'Active Mailer:'}
            </span>
          ...
          <div className="flex items-center space-x-1.5">
            {config.hasPassword ? (
              <span className="inline-flex items-center space-x-1 text-emerald-700 font-semibold bg-emerald-100/80 px-2 py-0.5 rounded-full text-[11px]">
                ...
                <span>{lang === 'zh' ? '伺服器端密碼已保存' : 'Password Stored'}</span>
              </span>
            ) : (
        ...
        <form onSubmit={handleSave} className="space-y-3.5 text-xs sm:text-sm">
          ...
          <div className="bg-amber-50/70 border border-amber-200/80 rounded-xl p-2.5 text-[11px] text-amber-900 flex items-start space-x-2">
            <HelpCircle className="w-4 h-4 text-amber-600 flex-shrink-0 mt-0.5" />
            <div className="leading-relaxed">
              <span className="font-bold">Google / Gmail 發信特別提示：</span>
              若使用 Google 信箱，請先至 Google 帳戶開啟「兩步驟驗證」，並於「安全性」頁面生成專屬的「
              <span className="font-semibold text-amber-800">應用程式密碼 (App Password)</span>
